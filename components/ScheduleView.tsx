...
import { Job, JobStatus, LoadingType, UserProfile, MainCategory, SubCategory, Personnel, Vehicle, UserRole, Priority, ConfirmationStatus } from '../types';
import { Plus, Search, MapPin, Package, Clock, User, X, Layers, Calendar as CalendarIcon, List, CheckCircle2, Truck, Settings2, Edit3, Lock, Unlock, Trash2, Users, Phone, Flag, ChevronDown } from 'lucide-react';
...
interface ScheduleViewProps {
  ...
  onUpdateJobStatus: (jobId: string, status: ConfirmationStatus) => void;
...
const PRIORITIES: Priority[] = ['Regular', 'Priority VIP', 'High Priority'];
const CONFIRMATION_OPTS: { value: ConfirmationStatus, label: string }[] = [
...
export const ScheduleView: React.FC<ScheduleViewProps> = ({ 
  jobs, onAddJob, onDeleteJob, onUpdateAllocation, onUpdateJobStatus, onToggleLock, currentUser, personnel, vehicles 
}) => {
  ...
  const getConfirmationBadgeClass = (status: ConfirmationStatus) => {
    switch (status) {
      ...
      case 'tentative': return 'bg-amber-50 text-amber-600 border-amber-100';
    ...
    <div className="space-y-6 animate-in slide-in-from-bottom-2 duration-500">
        ...
        {viewMode === 'calendar' ? (
          <div className="max-h-[70vh] overflow-y-auto custom-scrollbar">
             ...
             {HOURS.map((hour) => {
               ...
               return (
                 <div key={hour} className="grid grid-cols-[120px_1fr] border-b last:border-0 group min-h-[180px]">
                    ...
                    <div className="p-6 flex flex-wrap gap-6 items-start">
                       ...
                       {hourJobs.map((job) => (
                         <div key={job.id} className={`min-w-[340px] max-w-sm rounded-2xl p-6 border shadow-sm hover:shadow-md transition-all bg-white relative group/job ${job.isLocked ? 'border-amber-200' : 'border-slate-200'}`}>
                            {/* Color Indicator */}
                            <div className={`absolute top-0 left-0 w-1.5 h-full rounded-l-2xl ${getConfirmationColor(job.confirmationStatus || 'confirmed')}`}></div>
                            ...
                            <div className="space-y-3 mb-4">
                               ...
                               <div className="flex flex-col gap-2">
                                  <label className="text-[8px] font-bold uppercase text-slate-400 tracking-widest">Status Picker</label>
                                  <div className="flex items-center gap-2">
                                     <select 
                                       className={`px-3 py-1.5 rounded-xl text-[9px] font-bold uppercase border focus:ring-0 outline-none transition-all cursor-pointer ${getConfirmationBadgeClass(job.confirmationStatus || 'confirmed')}`}
                                       value={job.confirmationStatus || 'confirmed'}
                                       onChange={(e) => onUpdateJobStatus(job.id, e.target.value as ConfirmationStatus)}
                                     >
                                       {CONFIRMATION_OPTS.map(opt => (
                                         <option key={opt.value} value={opt.value}>{opt.label}</option>
                                       ))}
                                     </select>
                                     <span className="px-2 py-1.5 bg-slate-100 text-slate-600 border border-slate-200 rounded-xl text-[9px] font-bold uppercase">
                                       {job.priority}
                                     </span>
                                  </div>
                               </div>
            ...
            <table className="w-full text-left border-collapse min-w-[1000px]">
               <thead>
                  <tr className="bg-slate-50 text-slate-400 text-[10px] font-bold uppercase tracking-widest">
                     ...
                     <th className="p-6">Shipper / Contact</th>
                     <th className="p-6">Confirmation Status</th>
                     <th className="p-6">Priority / Allocation</th>
               ...
               <tbody className="divide-y divide-slate-100">
                  {filteredJobs.map(job => (
                    <tr key={job.id} className={`hover:bg-slate-50/50 transition-colors group relative ${job.isLocked ? 'bg-amber-50/5' : ''}`}>
                       <td className="p-6 text-sm font-bold text-blue-600">
                          <div className="flex items-center gap-3">
                            <div className={`w-3 h-3 rounded-full ${getConfirmationColor(job.confirmationStatus || 'confirmed')}`}></div>
                            {job.id}
                            {job.isLocked && <Lock className="w-3 h-3 text-amber-500" />}
                          </div>
                       </td>
                       <td className="p-6">
                         <div className="flex flex-col">
                            ...
                            <span className="text-[10px] text-slate-500 font-bold flex items-center gap-1.5 mt-1">
                       ...
                       <td className="p-6">
                          <div className="relative inline-flex group/status">
                            <select 
                              className={`appearance-none pr-8 pl-4 py-2 rounded-xl text-[10px] font-bold uppercase border cursor-pointer focus:ring-0 outline-none transition-all ${getConfirmationBadgeClass(job.confirmationStatus || 'confirmed')}`}
                              value={job.confirmationStatus || 'confirmed'}
                              onChange={(e) => onUpdateJobStatus(job.id, e.target.value as ConfirmationStatus)}
                            >
                               {CONFIRMATION_OPTS.map(opt => (
      ...
      {showModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-[2rem] w-full max-w-4xl shadow-2xl animate-in zoom-in-95 duration-300 flex flex-col max-h-[95vh] border border-slate-200 overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="p-8 overflow-y-auto space-y-8 custom-scrollbar">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input 
                      required 
                      type="tel" 
                      className="w-full px-5 py-3.5 pl-16 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold focus:ring-1 focus:ring-blue-500 outline-none" 
                      value={newJob.clientContact} 
                      onChange={e => {
                        let val = e.target.value;
                        if (!val.startsWith('+971 ')) val = '+971 ' + val.replace(/^\+971\s*/, '');
                        setNewJob({...newJob, clientContact: val});
                      }} 
                    />
                  </div>
                </div>
                ...
                <div className="space-y-1.5">
                   <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Job Priority *</label>
                   <select className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold focus:ring-1 focus:ring-blue-500 outline-none" value={newJob.priority} onChange={e => setNewJob({...newJob, priority: e.target.value as Priority})}>
                    {PRIORITIES.map(p => <option key={p} value={p}>{p}</option>)}
                  </select>
                </div>
                <div className="space-y-1.5">
                   <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Confirmation Status *</label>
                   <select className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold focus:ring-1 focus:ring-blue-500 outline-none" value={newJob.confirmationStatus} onChange={e => setNewJob({...newJob, confirmationStatus: e.target.value as ConfirmationStatus})}>
                    {CONFIRMATION_OPTS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                  </select>
                </div>
                <div className="space-y-1.5">
